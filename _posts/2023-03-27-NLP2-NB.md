...
This allows us to create the same table but instead of frequency, with probabilities for each word being in a positive or negative tweet. To do this, we have to pick each word and divide its’ frequency in a positive/negative context for the total words of that context. For example, the probability of word I being in a Positive tweet $P(”I”|Positive)$ is:
