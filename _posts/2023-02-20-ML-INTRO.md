...
But, what is $z$? $z$ is the value obtained when doing the linear regression explained on the other chapters: $f_{\overrightharpoon{w},b}(\overrightharpoon{x}) = \overrightharpoon{w} · \overrightharpoon{x} + b$ .

This implies that: 

 $g(z) = g(\overrightharpoon{w} · \overrightharpoon{x} + b) = \frac{1}{1+e^{-(\overrightharpoon{w} · \overrightharpoon{x} + b)}}$

To sum up, this function will use the output obtained with the liniar regression (multiple liniar regression) function, with parameters $\overrightharpoon{w}$ and b, and put transform it into a value between 0 and 1.
...
$f_{\overrightharpoon{w},b}(\overrightharpoon{x}) = P(y=1|x;\overrightharpoon{w},b)$

Assuming that we will predict that $\hat{y} = 1$ when $f_{\overrightharpoon{w},b}(\overrightharpoon{x})$  (logistic regression function) $**≥ 0.5**$, this implies the following. 

$f_{\overrightharpoon{w},b}(\overrightharpoon{x})$ will be $≥ 0.5$ if $g(z) ≥ 0.5$  because $f_{\overrightharpoon{w},b}(\overrightharpoon{x}) = g(z)$

Since $g(z) = \frac{1}{1+e^{-(\overrightharpoon{w} · \overrightharpoon{x} + b)}}$, it will be$≥ 0.5$ whenever $z ≥ 0$. 

Since $z = \overrightharpoon{w} · \overrightharpoon{x} + b$, it will be $≥ 0$  whenever $\overrightharpoon{w} · \overrightharpoon{x} + b$  is $≥ 0$. 

Having stated this, we could plot the decision boundary doing this (in this example where the threshhold is 0.5): 

$z = \overrightharpoon{w} · \overrightharpoon{x} + b = 0$
...
$w_j = w_j - \alpha \space \frac \partial {\partial w_j} J(w,b)$
 ...
 $b = b - \alpha \space \frac \partial {\partial b} J(\overrightharpoon{w},b)$
...
$\frac \partial {\partial b} J(\overrightharpoon{w},b) = \frac 1 {m} \sum_{i=1}^m (f_{w,b}(x^{(i)}) - y^{(i)})$ 
...
$\overrightharpoon{x} = [x_1, x_2, x_3, … , x_n]$ 
...
$f_{\overrightharpoon{w},b}(\overrightharpoon{x}) = \overrightharpoon{w}·\overrightharpoon{x} + b = w_1x_1 + w_2x_2 + … + w_n+x_n + b = (\sum_{j=1}^n w_jx_j) + b$

<aside>
ℹ️ Notice that $\overrightharpoon{w}·\overrightharpoon{x}$ is a **dot product**
...
You can see that w is now represented as $\overrightharpoon{w}$ too. This imply that it will be a vector too. Each feature $x_i$ will have it’s own parameter $w_i$.

### Gradient descent for multiple linear regression

The new cost function uses the vector notation for the w parameters: 

$J(\overrightharpoon{w},b)$
...
$w_j = w_j - \alpha \space \frac 1 {m} \sum_{i=1}^m (f_{\overrightharpoon{w},b}(\overrightharpoon{x}^{(i)}) - y^{(i)}) \space x_j^{(i)} \space for \space j = 0...n-1$

$b = b - \alpha \space \frac 1 {m} \sum_{i=1}^m (f_{\overrightharpoon{w},b}(\overrightharpoon{x}^{(i)}) - y^{(i)})$ 
...
![https://miro.medium.com/max/640/1*yi0VULDJmBfb1NaEikEciA.webp](https://miro.medium.com/max/640/1*yi0VULDJmBfb1NaEikEciA.webp)
...
So, the gradient descent algorithim is repeat until they converge: 
...
<aside>
⚠️ Remember de simultaeous update when coding this
