...
But, what is $z$? $z$ is the value obtained when doing the linear regression explained on the other chapters: $$ f_{\rightharpoonaccent{w},b}(\rightharpoonaccent{x}) = \rightharpoonaccent{w} · \rightharpoonaccent{x} + b $$

This implies that: 

 $g(z) = g(\rightharpoonaccent{w} · \overrightharpoon{x} + b) = \frac{1}{1+e^{-(\overrightharpoon{w} · \overrightharpoon{x} + b)}}$
