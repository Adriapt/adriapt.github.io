...
Web applications store the information in databases. You can imagine a database as a table with named columns where information is stored inside. When a user is navigating within the web application, he may want to be displayed for a specific information, such as "T-shirts" in a shopping webpage. To do this, the web application interacts with the database using SQL (Structured Query Language) queries. 
...
This may imply that the attacker can retrieve confidential data from that database and modify or delete it. It can even allow attackers execute remote code and gain access to the victim's server.  
...
We are able to change the ‘GIFTS’ value by writing whatever we want in the url, but we can’t directly interact with the RELEASED = ‘1’ part of the query. 

**Everything that we submit after the 'category=' will be included between two single quotes**, making it a string. However, if we are able to insert the single quote character ‘ in the input, we will be able to close the string wherever we want and add some code that will be executed. 
